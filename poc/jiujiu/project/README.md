...
- Character Agent uses the baseline's current physical-state data protocol `v2`, execution policy `v1`, and a bounded maximum of four activation cycles. The earlier explicit `v1` protocol failed rehearsal capability validation before a model call; that evidence is retained.
