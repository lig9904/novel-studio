...
标准：计划的当章实际动作、会面、地点、用时、已知事实与结果必须由最终arbitration、simulation及POV开章observation支持。角色提出/打算/期待/条件允许不代表动作已执行；承诺交付不等于送达，持有不等于读到，reported/estimated不等于世界精确真值。旧软大纲不能授权裁决未发生的事件。完成状态partial/in_progress不能写成完成。
	逐项核对contract.required_beats、continuity_checks、hook、causal_beats、render_capacity.scene_units、ending_consequence_contract、arc_transition_contract及可见对白/证据链。还必须检查会被未来Drafter正向消费的旁路：external_reference_plan.usable_details/transformation_rule、grounding_details.detail/transformed_as/scene_anchor、reality_support_plan.usable_detail/transformed_as/chapter_use、reader_entertainment_plan的opening/humor/payoff/companion、reader_reward_plan的正向奖励/代价/可见兑现、longform_opening的hook/engine/promise/seed/proof，以及literary_rendering_plan的scene_modes/active_lenses.move/afterimage。RAG/craft/web receipt只证明材料来源，不证明该人物、物件、设备、事件、通信、知识或结果已在故事中发生；这些正向字段仍须服从最终simulation/arbitration/POV evidence。输入中的实际路径分层不可省略：章节合同字段位于/plan/contract/...，因果推演字段位于/plan/causal_simulation/...；例如scene_units必须引用/plan/causal_simulation/render_capacity/scene_units/...，ending_consequence_contract必须引用/plan/causal_simulation/ending_consequence_contract/...。区分作者侧离屏约束、未来打算、历史回忆、禁止事项与正文当场发生的事实；query_or_need、source_refs、do_not_use、forbidden_*、reveal_budget、knowledge_boundary或说明“不得发生”的文本可以提及未发生内容，它们本身不等于计划宣告发生，不能因此误拒绝。无需逐字复述裁决，不因文学表达、比喻、微动作、合理感官细节或措辞不同而拒绝；这些细节不得增加实质决策/资源/信息/地点/时间变化。人物可作尚未验证的猜测，但计划必须明确其不确定性。
