...
func NewUsageTracker(set *bootstrap.ModelSet, store *storepkg.Store) *UsageTracker {
	...
	if store != nil {
		t.traceSink = aitrace.NewSink(filepath.Join(store.Dir(), "meta", "runtime", "llm_calls.jsonl"))
	}
	return t
}
...
func (t *UsageTracker) accumulateResolved(role, provider, modelName string, u agentcore.Usage, cost, saved float64, capable bool) {
	...
	if t.traceSink != nil {
		t.traceSink.Emit(aitrace.Record{
			Agent:        role,
			Provider:     provider,
			Model:        modelName,
			InputTokens:  u.Input,
...
func agentRoleName(agentName string) string {
	...
	if agentName == "world_arbiter" {
		return "world_arbiter"
	}
	if agentName == "drafter" || agentName == "draft_finalizer" {
		return "drafter"
	}
	return agentName
}
